...
/// a [BoolArray] (at most 2 distinct values per 1024-row chunk) instead of a numeric segment.
///
...
public record LazyRleBoolArray(
        DType dtype, long length, BoolArray values, MemorySegment indices, boolean wideIndices,
    ...
    public boolean getBoolean(long i) {
        ...
        return values.getBoolean(valueIdxOffset + Math.min(localIdx, numChunkValues - 1));
    }
    ...
    private void processChunk(int chunkIdx, int rowInChunk, int end, BooleanConsumer c) {
        ...
        if (numChunkValues <= 1) {
            ...
            for (int r = rowInChunk; r < end; r++) {
                ...
                c.accept(values.getBoolean(valueIdxOffset + Math.min(localIdx, maxIdx)));
            }
        } else {
            for (int r = rowInChunk; r < end; r++) {
