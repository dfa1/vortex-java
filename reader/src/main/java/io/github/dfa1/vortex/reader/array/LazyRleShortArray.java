...
///                          `i16` segment of exactly `valuesLen` elements
...
public record LazyRleShortArray(
    ...
    private short lookup(long i) {
        ...
        if (numChunkValues <= 1) {
            return numChunkValues == 1 ? values.getAtIndex(VortexFormat.LE_SHORT, valueIdxOffset) : (short) 0;
        }
        int localIdx = RleArrays.localIndex(indices, wideIndices,
                (long) chunkIdx * RleArrays.FL_CHUNK_SIZE + rowInChunk);
        return values.getAtIndex(VortexFormat.LE_SHORT,
    ...
    private long foldChunk(int chunkIdx, int rowInChunk, int end, long acc, LongBinaryOperator op) {
        ...
        if (numChunkValues <= 1) {
            short v = numChunkValues == 1 ? values.getAtIndex(VortexFormat.LE_SHORT, valueIdxOffset) : (short) 0;
            ...
            for (int r = rowInChunk; r < end; r++) {
                ...
                acc = op.applyAsLong(acc, widen(values.getAtIndex(VortexFormat.LE_SHORT,
