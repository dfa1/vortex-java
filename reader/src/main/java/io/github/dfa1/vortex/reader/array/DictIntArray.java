...
/// Dict-encoded [IntArray] view. ADR 0012 shape.
///
...
public record DictIntArray(DType dtype, long length, IntArray values, Array codes) implements IntArray {
    ...
    /// Builds a [DictIntArray], validating that `codes` is one of the
    ...
    /// little-endian `i32` segment. The codes switch is hoisted outside the loop so
    /// each branch is a uniform gather over a single code width.
    ///
    /// @param arena allocator for the output segment
    /// @return a read-only little-endian `i32` segment of gathered values
    ...
    public MemorySegment materialize(SegmentAllocator arena) {
        ...
        switch (codes) {
            ...
            case ShortArray sa -> {
                for (long i = 0; i < n; i++) {
                    dst.setAtIndex(VortexFormat.LE_INT, i, vals.getInt(Short.toUnsignedLong(sa.getShort(i))));
                }
            }
            case IntArray ia -> {
                for (long i = 0; i < n; i++) {
                    dst.setAtIndex(VortexFormat.LE_INT, i, vals.getInt(Integer.toUnsignedLong(ia.getInt(i))));
                }
            }
            case LongArray la -> {
                for (long i = 0; i < n; i++) {
                    dst.setAtIndex(VortexFormat.LE_INT, i, vals.getInt(la.getLong(i)));
                }
            }
