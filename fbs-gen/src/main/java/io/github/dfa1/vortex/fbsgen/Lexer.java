...
/// Tokenizes a FlatBuffers schema (`.fbs`) source file into a list of [Token]s.
/// Comments (`//` and `///` line, `/* ... */` block) and whitespace are dropped.
public final class Lexer {
    ...
    private Token readString() {
        ...
        if (pos >= src.length()) {
            throw new FbsParseException("unterminated string at line " + startLine);
        }
        pos++; // skip closing "
    ...
    private Token readPunct() {
        ...
        Token.Kind kind = switch (c) {
            ...
            case '.' -> Token.Kind.DOT;
            default -> throw new FbsParseException("unexpected character '" + c + "' at line " + line);
        };
    ...
    private void skipWhitespaceAndComments() {
        while (pos < src.length()) {
            ...
            if (c == '\n') {
                ...
                if (pos + 1 >= src.length()) {
                    throw new FbsParseException("unterminated block comment");
                }
                pos += 2;
            } else {
                return;
