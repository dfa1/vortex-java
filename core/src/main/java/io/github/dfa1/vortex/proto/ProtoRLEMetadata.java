...
public record ProtoRLEMetadata(
        long values_len,
        long indices_len,
        ProtoPType indices_ptype,
        long values_idx_offsets_len,
        ProtoPType values_idx_offsets_ptype,
        long offset
) {
    ...
    public static ProtoRLEMetadata decode(MemorySegment __seg, long __off, long __len) throws IOException {
        ProtoReader r = new ProtoReader(__seg, __off, __len);
        long values_len = 0;
        long indices_len = 0;
        ProtoPType indices_ptype = ProtoPType.U8;
        long values_idx_offsets_len = 0;
        ProtoPType values_idx_offsets_ptype = ProtoPType.U8;
        long offset = 0;
        while (r.hasMore()) {
            ...
            switch (tag >>> 3) {
                ...
                case 5 -> {
                    int __ev = r.readVarint32();
                    try {
                        values_idx_offsets_ptype = ProtoPType.fromValue(__ev);
                    } catch (IllegalArgumentException __iae) {
                        throw new IOException("unknown ProtoPType value: " + __ev);
                    }
                }
        ...
        return new ProtoRLEMetadata(values_len, indices_len, indices_ptype, values_idx_offsets_len, values_idx_offsets_ptype, offset);
    }
